...
This allows for a smooth integration and exchange of commercial and open source motion capture software to other parts of an entire production workflow. \\
Furthermore, we need benchmark datasets to measure and compare different approaches quantitatively. Appropriated methods can then be picked based on a certain use case. In addition, it enables accurate evaluations for new approaches.

\subsection{Motion Capture Tracking Format}

A simple solution to exchange tracking information is to store it as a set of certain keypoints in 2D or 3D coordinates. 
Afterwards, serialize it either as plain text or in binary file format. 
...
A mesh and skeleton based storage format limits the capabilities of some approaches. 
In order to predict entire 2D body surfaces including the movement of skin and fine facial details either a new mesh would need to be stored for each key frame or an increase of the skeleton count would be required. \\
...
Both offer along the actual storage and model mechanism tools to calculate a final mesh via vertex based linear blend skinning with learned corrective blend shapes.\\\\
The VRM File Format \cite{VRM_consortium} is based on glTF 2.0 and build specifically for exchanging humanoid 3D avatar data in VR/AR applications. This includes motion capture data in the form of a standardized bone structure as well as all information needed for an 3D application to create and render the 3D avatar.
This allows one format to be used within the entire lifecycle of the 3D model from its creation in the form of a 3D vertex mesh, texturing and rigging to the final usage in a 3D graphics engine application such as a game or video content. Rigging describes the process of mapping motion capture data to artistic animations of the character. While eye movements are tracked permanently via an artificial bone, emotions are controlled via toggles to allow certain face expressions defined by the artist. Secondary animation allow artists to create their own bones and associated weight parameters for the animation of hair, clothes or other accessories.
\cite{VRM_PDF} \cite{vrm_press_release}.\\
The VRM allows parties to exchange the model and their bone structure. However, it does not handle real time streaming of motion capture data. The VMC Protocol Specification \cite{vmc_protocol_specification} solves this gap by integrating the VRM format with Open Sound Control (OSC) \cite{OpenSoundControl}. OSC was originally developed as a better alternative to the MIDI format to transmit instrument data. 
This allows to package motion capture data into single UDP packages and stream it via Ethernet in real time. Applications supporting the VMC can be categorized into sender for bone or facial data and receiver for rendering or further processing the motion capture data often in combination with an VRM model.
This also allows receiver apps to decouple motion capture software from the actual rendering, which further improves software architecture.
VRM and VMC has seen adoption for a broad range of VTuber streaming software such as Warudo \cite{warudo_app} or VSeeFace \cite{vseeface_app}. Furthermore, we have also seen creators publishing their models in VRM on specific build platforms such as VRoidHub \cite{vroid_hub} or VGen \cite{vgen_vrm}.
Virtual worlds such as Vket \cite{vket_vrm} support the usage of such avatars in a real time VR experience.
...
The collection does not provide the original RGB videos or other sensor data, instead a custom computer generated render video is contained alongside the SMPL-X parameter for each sequence.
...
ACCAD \cite{AMASS_ACCAD} BMLhandball \cite{AMASS_BMLhandball} BMLmovi \cite{10.1371/journal.pone.0253157} BMLrub\cite{AMASS_BMLrub}
...
A variation of this is RMSPJPE (Root Mean Squared Per Joint Position Error) which uses the root-mean-square instead of a simple mean.\\
For approaches that output 2D surfaces or SMPL data MPVE (Mean Per Vertex Error) is used instead. This measures the error at each mesh vertex.

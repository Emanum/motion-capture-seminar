...
To give a brief overview of the research field we are presenting approaches with various sensors and methods.
While our focus lies on 3D pose estimation OpenPose \ref{method_openpose} with their real-time 2D tracking surves as a popular example in the industry with a large user base and also as base for other papers. 
DeMoCap \ref{method_democap} present their real-time marker based approach which uses low-cost depth sensors.
XNect \ref{method_xnect} can track multiple persons in real-time from a single camera similar to OpenPose but for a 3D skeleton.
Ultra Interal Poser \ref{method_uip} stands out as their only use wearable trackers attached to the body to generate 3D tracking data without any optical cameras.
MAMMA \ref{method_mamma} is a recent paper, presented in december 2025, which can compete with the golden standard for accuracy while being fully automated and markerless. 
While the paper is only available as a preprint, we still believe it is highly relevant due to their impressive results 
Furthermore, the authors are related to the Max Planck Institute for Intelligent Systems and Carnegie Mellon University from where we have seen other interesting approaches in the past. 

\subsection{OpenPose - Marker less 2D}
\label{method_openpose}
...
Each network describes a stage, the output of one stage is used as input for the next stage. By using a fully differentiable model backpropagation can be used for training. One issue with their approach is handling multiple people in the same image.\\
This is addressed in their next publication with Part Affinity Fields \cite{openPose_3_cao2017realtime}. A bottom-up approach is used. This means that first body parts are detected and afterwards assigned to an unknown number of people in the image.
...
In \cite{openPose_2_simon2017hand}, Simon et al. propose a method that produces 3D motion capture results handling complex Occlusion scenarios. They build up on their previous models to predict 2D estimation via multiple camera angles. The result is then triangulated to get 3D results. 
...
While their methods performs quite good even under complex scenarios on public datasets such as the SFU Dataset \cite{sfudataset} it comes with some serious limitations. 
...
Xnect \cite{mehta_xnect_2020} is a real-time marker less approach that is powered by a single monocular RGB based camera and can provide temporally coherent 3D tracking for multiple people in diverse scenes in the wild.

Their bottom up architecture start by applying a convolutional neural network which is trained to detect only fully visible features such as the joint itself, or it's parent/child. The networks outputs are 2D body joint heatmaps, part affinity fields to assign joints to individual persons as well as their own 3D pose encoder which uses local pixel information close to the 2D joint position. 
...
Lastly, a space-time skeleton model is applied to further improve the 3D pose. 
...
The end result of the pipeline is a full skeleton pose with joint angles for each person.

While their CNN applies a novel CNN architecture called SelecSLS to achieve real-time performance, it is mentioned that any CNN architecture for keypoint prediction can be used.
This makes their approach potentially be extended in the future by upcoming CNN network architectures. 

The results are compared to Microsoft Kinect V2 which uses depth sensing cameras with similar and sometimes better qualitative performance especially for scenes including occlusions. 
In addition, results on common datasets such as MuPoTS-3D benchmark dataset \cite{singleshotmultiperson2018} and the Panoptic dataset \cite{Joo_2017_TPAMI} are presented and compared to SingleShot \cite{singleshotmultiperson2018}, LCRNet \cite{shi2024tro}, MP3D \cite{mp3d}, LCRNet++ \cite{lcrnetplusplus} and PoseNet \cite{resnet}. The only approach that had a better score was PoseNet, however it comes with other limitations one of which is the lack of real-time performance. Their own CNN SelecSLS is compared to ResNet \cite{resnet} with similar results while still also lacking real-time capabilities.
However, compared to multi-view capture systems XNect has far less accuracy. 
While occlusion is partly handled via their 2nd stage, it is far from perfect. 
Occlusion of the neck of a person result in the entire person not being detected regardless if large parts of the body are visible. Close human interactions like hugging also leads to inaccurate results. 
...
\subsection{Ultra Inertial Poser - 3D IMU based}
\label{method_uip}
...
UWB signals use a large bandwidth together with short waveforms, based on that physical property there exist multiple protocols used in signal processing to measure distance: Two-Way Ranging, Time Difference of Arrival and Angle of Arrival \cite{UWB_Sewio}.
...
It is stated that the IMU sensor quality highly influences drift errors. As other methods performed way worse on their dataset gathered with inexpensive IMU than synthetic datasets or data gathered from high-end sensors.
...
However, they do not compare their approach against camera based tracking methods, which are way less prone to jitter. 
A video presentation \cite{UltraInertialPoser_YT} of their work shows a clear improvement compared to pure IMU based methods. Nevertheless, there is still a large jitter movement visible to the naked eye.
...
\subsection{MAMMA - offline 3D Gold Standard Competitor}
\label{method_mamma}
...
Their network is trained on a pure synthetic dataset which simulates their 33 camera studio setup, consisting of single person scenes, complex interaction scenes including dance scenes with two persons and a subset focusing on hand articulations. \\
...
Furthermore, a direct comparison to Vicon \cite{VICON} is performed using a real evaluation dataset. Results show that there is only a minimal 1.611mm difference compared to the "gold standard". This make their technique accurate enough for many applications. An interesting comparison is also done in terms of usability and cost. Capturing a dataset with Vicon requires manual post-processing. 3 skilled technicians worked in total close to 47 hours to perform the cleanup of a 24-minute scene. Computation for the final SMPL-X format took another 25 hours. This is significantly longer compared to MAMMA which does not require manual post-processing and took only about 8 hours on an RTX-4090 to compute the final motion tracking result.\\
...
% EgoPoser \cite{jiang2024egoposer}\\

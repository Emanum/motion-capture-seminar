...
Human motion capture is used in the movie industry for computer generated imagery (CGI), dating back all the way to an animation technique called rotorscoping in 1915 by the cartoonist Max Fleischer \cite{menache_understanding_2000}. While the implementation of these techniques got modernized from a projected movie on a piece of paper to optical sensor based systems, the goal stayed the same: Capturing real world human motion data and translating them into a virtual representation for further processing such as in traditional 2D hand-drawn animations or modern 3D computer generated animations. \\
To achieve as much accuracy as possible, traditional state-of-the-art motion capturing studios use suits equipped with markers together with large quantity of high resolution and high frame rate camera systems in a multi view setup combined. This is often referred to as the ``gold standard`` in literature \cite{cuevas2025mamma}.
...
Therefore, many state-of-the-art motion capture systems in the industry and approaches in research use numerous cameras to cover as many angles as possible.
...
In chapter \ref{chapter_task} our task is presented which focus around real time motion capture for live concerts with virtual avatars. 
In chapter \ref{chapter_discussion} we categorize and compare approaches. 

...
Traditional "gold standard" motion capture studio use marker based methods to produce sub-millimeter accurate tracking. MAMMA (Markerless and Accurate Multi-person Motion Automatic capture) \cite{cuevas2025mamma} claim to achieve similar results with a similar studio setup including 32 fixed mounted RGB cameras. However, their method do so via a markerless approach, therefore reducing setup and post-processing duration time. 
Their algorithm takes various number of camera feeds as input and estimate a dense 2D surface landmarks in the form of the SMPL-X format \cite{SMPLX}. 
A network consisting of vision transformer (ViT) and a CNN is trained to extract images features. They use a technique called landmark queries, which means that instead of using a single embedding multiple embeddings are used to further increase generalization of the network. Therefore also increasing accuracy for challenging poses that are not present in the training data. This is possible as their technique uses the entire video data as input instead of just marker positions. 
The features of the ViT is combined with the CNN features and used in a Transformer Decoder and MLP network to predict 2D landmarks for each camera view with a probability value for the visibility. Which can be compared to marker locations in other approaches. 
In a final stage these landmarks are fit to a SMPL-X 3D body including multiple stages of optimization.
They train their network on a pure synthetic dataset which simulates their 32 camera studio setup. It consists of single person scenes, complex interaction scenes including dance scenes with two persons and a subset focused on hand articulations. 
An evaluation on the Hi4D dataset shows that their method outperform other approaches. The best in class, Avatarpose \cite{leonardis_avatarpose_2025} was beaten in tracking accuracy with a value of 13.43 to 32.10 Mean Per-Joint Position Error (mm).
Furthermore, a direct comparison to Vicon \cite{VICON} is done using a real evaluation dataset. Their results show that there is only a minimal 1.611mm difference compared to the "gold standard". Which make their technique accurate enough for many applications. An interesting comparison is also done in terms of usability and cost. Capturing a dataset with Vicon requires manual post-processing. On their 24-minute dataset it took 3 skilled technicians in total close to 47 hours to perform the cleanup and labeling. Computation of these results took another 25 hours until the final SMPL-X format. Their trained network does not require manual post-processing and took about 8 hours on an RTX-4090 to compute.

...
First a LSTM network together with a distance attention graph convolutional network is used to determine the positions of the trackers in 3D space.
Afterwards a kinematics decoder based on the work Physical Inertial Poser \cite{PIPCVPR2022} is used to estimate the final SMPL pose parameters as well as a global translation relative to an initial T-Pose calibration performed by the user.

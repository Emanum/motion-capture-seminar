...
Furthermore, we also added 3 more interesting approaches that were not discussed in details in 
Chapter \ref{chapter_methods}
Table \hyperref[tab:dataset]{2} compares the approaches in terms of tracking accuracy (MPJPE).\\\\
...
In general, we can see that optical camera based systems are preferred due to lower susceptibility to jitter and drift errors compared to IMU sensors as seen in Ultra Inertial Poser \cite{UltraInertialPoser}. 
The limitations in distance and refresh for depth sensors as seen in DeMoCap \cite{chatzitofis2021democap} further suggest the usage of optical cameras instead.
...
In order to use existing 3D models during a live show support for the VRM format is unavoidable.
...
Methods design to work in the wild like PACE \cite{PACE} or EgoPoser \cite{jiang2024egoposer} are impressive but seem to be more suited for wearable AR/VR devices than multi-person tracking in a studio setup.\\
The combination of multi person tracking during a realtime scenarios still serves as a great challenge as our literature review shows. From the 8 approaches in Table \hyperref[tab:comparison]{1} only XNect \ref{method_xnect} fulfilled both of these requirements.
However, if we ignore the realtime aspect MAMMA \cite{cuevas2025mamma} and Avatarpose \cite{leonardis_avatarpose_2025} seems to be the best approaches for our use-case. 
In their current setup MAMMA focus on tracking accuracy and applies 33 cameras which is considerable higher than other methods that work in realtime.
Therefore, we hope that we will see upcoming approaches with focusing on realtime instead of tracking accuracy while still allowing to track multiple people.
...
\vspace{0.5cm}
\textbf{Table 1:} Qualitative Comparisons of the approaches including their general sensor setup and key limitations.
...
\vspace{0.5cm}
\textbf{Table 2:} Quantitative comparison of the approaches for tracking accuracy. Values represent Mean Per-Joint Position Error (MPJPE) in millimeters. Note that the evaluation on each approach was not standardized due to the large difference of the approaches. So the quantitative numbers should only be taken as a relative comparison between the approaches. For detailed benchmark results refer to the individual papers.
\label{tab:dataset}
